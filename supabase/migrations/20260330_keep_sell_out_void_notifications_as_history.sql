...
begin
  ...
  then
    v_title := case
      when new.status = 'approved' then 'Pengajuan batal sudah di-approve'
      else 'Pengajuan batal sudah ditolak'
    end;
    v_body := case
      when new.status = 'approved' then format(
        'Pengajuan batal penjualan %s di %s sudah Anda approve.',
        v_product_name,
        v_store_name
      )
      else format(
        'Pengajuan batal penjualan %s di %s sudah Anda tolak.',
        v_product_name,
        v_store_name
      )
    end;
