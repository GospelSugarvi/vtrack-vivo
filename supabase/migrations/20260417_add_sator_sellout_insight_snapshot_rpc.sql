create or replace function public.get_sator_sellout_insight_snapshot(
  p_sator_id uuid,
...
begin
  ...
  promotor_rows as (
    select
      ...
      coalesce(nullif(u.nickname, ''), u.full_name, 'Promotor') as name,
    ...
    left join public.users u on u.id = ps.promotor_id
    left join lateral (
      ...
      coalesce(
        (
          select jsonb_agg(
            ...
            order by pr.store_name, pr.name
          )
...
grant execute on function public.get_sator_sellout_insight_snapshot(uuid, date)
to authenticated;
