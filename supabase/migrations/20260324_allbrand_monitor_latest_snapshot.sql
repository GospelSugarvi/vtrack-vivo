...
returns table (
  ...
  latest_submitted_by_id uuid,
  latest_submitted_by_name text,
  latest_submitted_at timestamptz,
  latest_daily_total_units integer,
  latest_cumulative_total_units integer,
  status text
)
language sql
security definer
...
as $$
  ...
  select
    ...
    latest_rpt.id as latest_report_id,
    latest_rpt.report_date as latest_report_date,
    latest_rpt.promotor_id as latest_submitted_by_id,
    latest_u.full_name as latest_submitted_by_name,
    coalesce(latest_rpt.updated_at, latest_rpt.created_at) as latest_submitted_at,
    coalesce(latest_rpt.daily_total_units, 0) as latest_daily_total_units,
    coalesce(latest_rpt.cumulative_total_units, 0) as latest_cumulative_total_units,
    case
  ...
  left join lateral (
    ...
    where ar.store_id = ast.store_id
      and ar.report_date <= p_date
    order by ar.report_date desc, ar.updated_at desc nulls last, ar.created_at desc nulls last, ar.id desc
    limit 1
  ) latest_rpt on true
  left join public.users latest_u
    on latest_u.id = latest_rpt.promotor_id
  order by
