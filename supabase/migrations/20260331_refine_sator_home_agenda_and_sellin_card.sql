...
begin
  ...
  v_total_approval_pending := coalesce(v_schedule_pending_count, 0) + coalesce(v_permission_pending_count, 0);
  ...
  left join public.stores st on st.id = sv.store_id
  ...
  select coalesce(
    json_agg(item order by sort_order),
    '[]'::json
  )
  into v_agenda
  from (
    select
      ...
      json_build_object(
        'type', 'visiting',
        'title', 'Visiting',
        'sub', coalesce(v_visit_store_name, 'Belum visit hari ini'),
        'status', case when v_visit_store_name is null then 'pending' else 'done' end
      ) as item
    union all
    select
      2 as sort_order,
      json_build_object(
        ...
        'sub',
          case
            when v_sellin_pending_count > 0 then v_sellin_pending_count::text || ' order pending'
            else 'Tidak ada order pending'
          end,
        'status', case when v_sellin_pending_count > 0 then 'process' else 'ok' end
      ) as item
    union all
    select
      3 as sort_order,
      json_build_object(
        'type', 'approval',
        'title', 'Approval Sator',
        'sub',
          case
            when v_total_approval_pending > 0 then v_total_approval_pending::text || ' approval belum dikerjakan'
            else 'Tidak ada approval tertunda'
          end,
        'status', case when v_total_approval_pending > 0 then 'pending' else 'ok' end
      ) as item
  ) agenda_rows;
