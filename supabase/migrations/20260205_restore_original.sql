-- ==========================================================
-- RESTORE: Function asli yang sudah jalan sebelum saya rusak
-- ==========================================================

-- Drop dulu yang broken
DROP FUNCTION IF EXISTS get_store_stock_status(uuid);

-- Restore function yang asli (dari file fix_sator_functions_use_store_assignment_v2.sql)
...
BEGIN
  ...
  LEFT JOIN LATERAL (
    SELECT 
      ...
      COUNT(*) FILTER (WHERE COALESCE(si.quantity, 0) > 0 AND COALESCE(si.quantity, 0) < 5) as low_count,
      COUNT(*) FILTER (WHERE COALESCE(si.quantity, 0) >= 5) as ok_count
