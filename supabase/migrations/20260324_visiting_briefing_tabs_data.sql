...
begin
  ...
  activity_by_promotor as (
    select
      ...
      coalesce((
        select ut.target_omzet
        from public.user_targets ut
        where ut.user_id = ap.promotor_id
        limit 1
      ), 0)::int as monthly_target_omzet,
      round(coalesce((
        select ut.target_omzet
        from public.user_targets ut
        where ut.user_id = ap.promotor_id
        limit 1
      ), 0) / 30.0)::int as daily_target,
      round(coalesce((
        select ut.target_fokus_total
        from public.user_targets ut
        where ut.user_id = ap.promotor_id
        limit 1
      ), 0) / 30.0)::int as focus_target,
      coalesce((
