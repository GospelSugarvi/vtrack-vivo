-- =====================================================
-- FIX CHAT SYSTEM V3 (STRICT ALIASING)
-- Date: 30 January 2026
-- Description: Fixes "ambiguous column reference" error (42702) by enforcing
--              strict table aliasing on ALL columns. Consolidates previous fixes.
-- =====================================================
...
DROP FUNCTION IF EXISTS send_message(UUID, UUID, VARCHAR, TEXT, TEXT, UUID[], UUID, INTEGER, INTEGER); -- Drop variations
DROP FUNCTION IF EXISTS mark_messages_read(UUID, UUID, UUID);

-- =====================================================
-- 2. GET USER CHAT ROOMS (Fixed Ambiguous References)
-- =====================================================
...
BEGIN
    ...
    SELECT 
        ...
        -- Count unread messages (Explicit aliasing)
        COALESCE((
            SELECT COUNT(*)::BIGINT
        ...
        -- Last message content (Explicit aliasing)
        (
            SELECT msg.content
            FROM chat_messages msg
        ...
        -- Last message time (Explicit aliasing - CRITICAL FIX for ambiguous error)
        (
        ...
        -- Last message sender (Explicit aliasing)
        (
        ...
        -- Member count (Explicit aliasing)
        (
            SELECT COUNT(*)::BIGINT
...
-- =====================================================
-- 3. GET CHAT MESSAGES 
-- Fixed avatar_url and ambiguous references
-- =====================================================
...
BEGIN
    ...
    SELECT 
        ...
        -- Reply info with explicit aliasing
...
-- =====================================================
-- 4. SEND MESSAGE (Matches Flutter)
-- =====================================================
