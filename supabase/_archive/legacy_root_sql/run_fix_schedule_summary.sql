-- Run the fix for get_sator_schedule_summary function

-- Drop and recreate the function with correct column name
DROP FUNCTION IF EXISTS get_sator_schedule_summary(UUID, TEXT);
...
BEGIN
    ...
    SELECT 
        ...
        STRING_AGG(DISTINCT st.store_name, ', ') as store_name, -- FIXED: st.name -> st.store_name
        COALESCE(
            (
                SELECT sch.status 
                FROM schedules sch 
...
COMMENT ON FUNCTION get_sator_schedule_summary IS 'Get schedule status summary for all promotors under SATOR (FIXED: use store_name column)';
