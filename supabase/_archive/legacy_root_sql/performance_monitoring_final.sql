-- PERFORMANCE MONITORING - FINAL VERSION (TESTED)
-- Using correct column names for Supabase
-- ==========================================

-- 1. INDEX EFFICIENCY MONITORING (CORRECT COLUMN NAMES)
...
-- 2. TABLE BLOAT MONITORING (CORRECT COLUMN NAMES)
...
-- 3. TABLE SIZE MONITORING (CORRECT COLUMN NAMES)
...
-- 5. PERFORMANCE ALERT SYSTEM (SIMPLIFIED & WORKING)
...
BEGIN
    ...
    SELECT 
        'UNUSED_INDEX'::TEXT,
        '🟡 MEDIUM'::TEXT,
        format('Index %s.%s is unused (%s scans)', schemaname, indexrelname, idx_scan)::TEXT,
    ...
    SELECT 
        'TABLE_BLOAT'::TEXT,
        '🟠 MEDIUM'::TEXT,
        format('Table %s.%s has %s%% bloat', schemaname, relname, 
    ...
    SELECT 
        'VACUUM_NEEDED'::TEXT,
        '🟡 MEDIUM'::TEXT,
        format('Table %s.%s has not been vacuumed recently', schemaname, relname)::TEXT,
...
-- 7. TARGET DASHBOARD PERFORMANCE TEST
...
-- 9. QUICK PERFORMANCE CHECK (WORKING VERSION)
...
-- Check table sizes (using correct column names)
SELECT 
