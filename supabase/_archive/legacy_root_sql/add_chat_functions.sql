-- =====================================================
-- CHAT MAIN FUNCTIONS (Missing functions for Flutter)
-- =====================================================

-- 1. GET USER CHAT ROOMS
    ...
    COALESCE(
      (SELECT COUNT(*) FROM chat_messages msg 
       ...
       AND msg.created_at > COALESCE(
         (SELECT last_read_at FROM chat_room_members WHERE room_id = cr.id AND user_id = p_user_id),
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM chat_room_members WHERE room_id = p_room_id AND user_id = v_sender_id) THEN
    RAISE EXCEPTION 'User is not a member of this room';
  END IF;
  
  -- Insert message
  INSERT INTO chat_messages (room_id, sender_id, content, message_type, attachment_url)
...
BEGIN
  ...
  IF v_room_id IS NULL THEN
    ...
    INSERT INTO chat_room_members (room_id, user_id) VALUES (v_room_id, v_user_id);
    INSERT INTO chat_room_members (room_id, user_id) VALUES (v_room_id, p_other_user_id);
  END IF;
