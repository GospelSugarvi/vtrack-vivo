...
class PermissionApprovalPage extends StatefulWidget {
  const PermissionApprovalPage({super.key});
...
class _PermissionApprovalPageState extends State<PermissionApprovalPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      final snapshotRaw = await _supabase.rpc(
        'get_sator_permission_approval_snapshot',
      );
      ...
      debugPrint('Error loading permission approvals: $e');
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _processRequest(String requestId, String action) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: Text(
            action == 'approve' ? 'Approve Izin' : 'Tolak Izin',
    ...
    try {
      final result = await _supabase.rpc(
        'process_permission_request_by_sator',
        params: {
  ...
  List<Map<String, dynamic>> get _filteredRows {
    if (_tab == 'pending') {
      return _rows
          .where((row) => '${row['status']}' == 'pending_sator')
          .toList();
    }
    return _rows.where((row) => '${row['status']}' != 'pending_sator').toList();
  }
  ...
  Widget _buildEmpty() {
    return Container(
      ...
      child: Text(
        _tab == 'pending'
            ? 'Belum ada izin yang menunggu approval SATOR.'
            : 'Belum ada riwayat approval perijinan.',
        style: PromotorText.outfit(
          size: 14,
          weight: FontWeight.w700,
  ...
  Widget _buildCard(Map<String, dynamic> row) {
    ...
    final spvName = '${row['spv_name'] ?? 'SPV'}';
    return Container(
