...
class _SatorVastPageState extends State<SatorVastPage> {
  ...
  int get _monthlyTeamTarget {
    final total = _monthlyTeamRows.fold<int>(
      0,
  ...
  Future<void> _loadSummary() async {
    ...
    final daily = await _supabase
        .from('vast_agg_daily_sator')
        .select()
        .eq('sator_id', userId)
        .eq('metric_date', today)
        .maybeSingle();
    final weekly = await _supabase
        .from('vast_agg_weekly_sator')
        .select()
        .eq('sator_id', userId)
        .eq('week_start_date', weekStartStr)
        .maybeSingle();
    final monthly = await _supabase
        .from('vast_agg_monthly_sator')
        .select()
        .eq('sator_id', userId)
  ...
  Future<void> _loadTeamRows() async {
    final userId = _supabase.auth.currentUser?.id;
    if (userId == null) return;
    final now = DateTime.now();
    final today = DateFormat('yyyy-MM-dd').format(now);
    ...
    var promotors = hierarchyRows
        .where((row) => '${row['sator_id'] ?? ''}' == userId)
        .toList();
    if (promotors.isEmpty) {
      final relationRows = List<Map<String, dynamic>>.from(
        await _supabase
      ...
      final fallbackPromotorIds = relationRows
    ...
    List<Map<String, dynamic>> buildRows(
      ...
      final rows = promotors.map((row) {
        ...
        return <String, dynamic>{
          ...
          'duplicate_alerts': _toInt(source['total_duplicate_alerts']),
          'total_acc': _toInt(source['total_acc']),
          'total_reject': _toInt(source['total_reject']),
          'promotor_with_input': _toInt(source['promotor_with_input']),
    ...
    _dailyTeamRows = buildRows(dailyById, 'daily');
    _weeklyTeamRows = buildRows(weeklyById, 'weekly');
    _monthlyTeamRows = buildRows(monthlyById, 'monthly');
  }
                ...
                ConstrainedBox(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) =>
                        _buildPromotorTableRow(teamRows[index]),
                  ),
                ),
              ],
            ),
